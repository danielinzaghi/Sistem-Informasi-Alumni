<x-app-layout>
    ...
    <div class="flex justify-between items-start gap-4 flex-col sm:flex-row">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b cursor-pointer jurusan-row" data-id="{{ $j->id }}">
                        ...
                        <td class="px-2 text-center py-4 flex justify-center items-center">
                            ...
                            <button data-id ="{{ $j->id }}" type="button" class="edit-user-btn text-yellow-400 border border-yellow-400 hover:bg-yellow-400 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-md text-sm p-2 text-center inline-flex items-center me-2">
        ...
        <div class="w-full sm:w-2/3 bg-white p-4 flex flex-col gap-4">
            <div class="flex justify-between items-center">
                <span class="w-full text-md sm:text-2xl text-center font-bold">Daftar Program Studi</span>
                <span class="w-2/3 text-center font-bold">button</span>
            </div>
            ...
            <table class="w-full sm:w-100 sm:min-w-full text-sm text-left text-gray-500 border border-gray-300 shadow-md rounded-lg">
                ...
                <tbody id="programStudiTable">
                    <tr>
                        <td colspan="3" class="text-center py-4">Pilih jurusan terlebih dahulu</td>
                    </tr>
                </tbody>
            </table>
        </div>
                            ...
                            $.each(response, function(index, program) {
                                programTable.append(`
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="px-2 text-center py-4 flex justify-center items-center">
                                            <button data-id ="${program.id}" type="button" class="edit-user-btn text-yellow-400 border border-yellow-400 hover:bg-yellow-400 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-md text-sm p-2 text-center inline-flex items-center me-2">
                                                <svg class="w-4 sm:w-6 h-4 sm:h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                                    <path fill-rule="evenodd" d="M11.32 6.176H5c-1.105 0-2 .949-2 2.118v10.588C3 20.052 3.895 21 5 21h11c1.105 0 2-.948 2-2.118v-7.75l-3.914 4.144A2.46 2.46 0 0 1 12.81 16l-2.681.568c-1.75.37-3.292-1.263-2.942-3.115l.536-2.839c.097-.512.335-.983.684-1.352l2.914-3.086Z" clip-rule="evenodd"/>
                                                    <path fill-rule="evenodd" d="M19.846 4.318a2.148 2.148 0 0 0-.437-.692 2.014 2.014 0 0 0-.654-.463 1.92 1.92 0 0 0-1.544 0 2.014 2.014 0 0 0-.654.463l-.546.578 2.852 3.02.546-.579a2.14 2.14 0 0 0 .437-.692 2.244 2.244 0 0 0 0-1.635ZM17.45 8.721 14.597 5.7 9.82 10.76a.54.54 0 0 0-.137.27l-.536 2.84c-.07.37.239.696.588.622l2.682-.567a.492.492 0 0 0 .255-.145l4.778-5.06Z" clip-rule="evenodd"/>
                                                </svg>                                                    
                                            ...
                                            <form action="action="/admin/prodi/${program.id}"" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    data-confirm-delete="true"
